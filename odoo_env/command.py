...
class WriteConfigFile(Command):
    ...
    def execute(self):
        ...
        try:
            ...
            msg.err('Corrupted odoo.conf :%s' % str(ex))

        try:
            admin_passwd = config_parser.get('options', 'admin_passwd')
        except Exception as ex:
            admin_passwd = False

        path_in_list = list(filter(lambda x: x.find('addons_path') >= 0, config))
        if path_in_list:
            config.remove(path_in_list[0])
        config.append('addons_path = %s\n\n' % ','.join(repos))
        if admin_passwd:
            config.append('admin_passwd = %s' % admin_passwd)

        with open(client.config_file, 'w') as f:
            f.write('; Configuration file generated by odoo-env v%s\n\n' % __version__)
            f.write('[options]\n')
